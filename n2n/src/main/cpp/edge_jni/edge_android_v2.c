...
static int
build_unicast_arp(char *buffer, size_t buffer_len, uint32_t target, n2n_android_t *priv) {
...
int establishVpnService(n2n_edge_cmd_t *cmd, char ipStr[], int netmask) {
    ...
    /* Call VpnService protect */
    jmethodID midEstablishVpnService = (*env)->GetMethodID(env, vpn_service_cls,
                                                           "EstablishVpnService",
    ...
    int vpn_fd = (*env)->CallIntMethod(env, g_status->jobj_service, midEstablishVpnService, jIpStr,
                                       netmask);
    if (vpn_fd < 0) {
